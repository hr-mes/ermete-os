...
if [[ -n "$DIR" && -d "$DIR" ]]; then
  ...
  if [[ -n "$UPSTREAM_VER" ]]; then
    CONTENT_HASH=$(echo -n "${PACKAGE}-${UPSTREAM_VER}-${BASE_DIGEST}-v6" | sha256sum | awk '{print $1}')
  else
    CONTENT_HASH=$(echo -n "${PACKAGE}-${VERSION}-upstream-v6-${BASE_DIGEST}" | sha256sum | awk '{print $1}')
  fi
fi

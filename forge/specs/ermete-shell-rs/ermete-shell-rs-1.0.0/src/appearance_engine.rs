...
impl AppearanceEngine {
    ...
    pub fn load_custom_presets_from_disk(&self) {
        ...
        if let Ok(home) = std::env::var("HOME") {
            dirs.push(PathBuf::from(home).join(".config").join("ermete").join("layouts"));
        }
        dirs.push(PathBuf::from("/usr/share/ermete/layouts"));
    ...
    fn get_active_layout_path() -> PathBuf {
        if let Ok(home) = std::env::var("HOME") {
            PathBuf::from(home).join(".config").join("ermete").join("active_layout.toml")
        } else {
            PathBuf::from("/tmp/ermete_active_layout.toml")
        }
    }
...
fn hot_swap_gtk_layer_shell(preset: &LayoutPreset) {
    ...
    if !preset.dock.enabled {
        let _ = gtk4::glib::spawn_command_line_async("pkill -STOP ermete-dock");
    } else {
        let _ = gtk4::glib::spawn_command_line_async("pkill -CONT ermete-dock");
    }
}

/// DBus interface implementation for `org.ermete.Shell.Layout`
...
#[interface(name = "org.ermete.Shell.Layout")]
impl LayoutDbusServer {
...
/// Register `org.ermete.Shell.Layout` DBus interface on session bus
pub async fn register_layout_dbus(connection: &zbus::Connection, engine: Arc<AppearanceEngine>) -> zbus::Result<()> {
    ...
    connection.object_server().at("/org/ermete/Shell/Layout", server).await?;
    connection.request_name("org.ermete.Shell.Layout").await?;
    info!("Registered DBus service 'org.ermete.Shell.Layout' at path '/org/ermete/Shell/Layout'");
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn test_builtin_preset_windows_classic() {
        let preset = LayoutPreset::windows_classic();
        assert_eq!(preset.name, "windows-classic");
        assert_eq!(preset.panel.position, PanelPosition::Bottom);
        assert_eq!(preset.panel.height_or_width, 44);
        assert!(!preset.dock.enabled);
    }

    #[test]
    fn test_builtin_preset_macos_glass() {
        let preset = LayoutPreset::macos_glass();
        assert_eq!(preset.name, "macos-glass");
        assert_eq!(preset.panel.position, PanelPosition::Top);
        assert_eq!(preset.panel.height_or_width, 28);
        assert!(preset.dock.enabled);
    }
    ...
    fn test_toml_serialize_deserialize_roundtrip() {
        let preset = LayoutPreset::macos_glass();
        let toml_str = serialize_preset_toml(&preset).expect("Serialization failed");
        assert!(toml_str.contains("macos-glass"));

        let parsed = parse_preset_toml(&toml_str).expect("Deserialization failed");
        assert_eq!(preset, parsed);
    }

    #[test]
    fn test_appearance_engine_preset_switching() {
        let engine = AppearanceEngine::new();
        assert_eq!(engine.get_active_layout_name(), "macos-glass");

        let res = engine.set_layout_by_name("windows-classic");
        assert!(res.is_ok());
        assert_eq!(engine.get_active_layout_name(), "windows-classic");

        let presets = engine.list_preset_names();
        assert!(presets.contains(&"windows-classic".to_string()));
        assert!(presets.contains(&"macos-glass".to_string()));
    }

    #[test]
    fn test_custom_toml_parsing() {
        let custom_toml = r#"
name = "custom-cyberpunk"
description = "Custom layout preset"
style_variant = "cyberpunk"
...
layout_grid = "minimal"
"#;
        let preset = parse_preset_toml(custom_toml).expect("Parsing custom TOML failed");
        assert_eq!(preset.name, "custom-cyberpunk");
        assert_eq!(preset.panel.position, PanelPosition::Top);
        assert_eq!(preset.panel.height_or_width, 36);
        assert_eq!(preset.panel.margin_top, 4);
    }
}

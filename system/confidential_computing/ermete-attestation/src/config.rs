...
impl Default for AttestationConfig {
    fn default() -> Self {
        Self {
            remote_pubkey_path: PathBuf::from("/etc/ermete/attestation/remote_pubkey.pem"),
            key_output_path: PathBuf::from("/run/ermete/var_home.key"),
...
impl AttestationConfig {
    pub fn load_or_default() -> Self {
        let env_path = std::env::var("ERMETE_ATTESTATION_CONFIG").ok().map(PathBuf::from);
        let config_path = env_path.unwrap_or_else(|| PathBuf::from("/etc/ermete/attestation/config.json"));
